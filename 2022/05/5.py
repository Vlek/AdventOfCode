...
with open("5_input.txt", "r") as line_text:
    ...
    for line in line_text:
        ...
        # Note, this is happy path only. If we were going to rely on
        # this in a production environment, we would need to ensure that
        # the rows both exist and that the row we are pulling from has
        # enough crates to pull.
        for num_crates in range(number_to_move):
            crate_stacks[crate_row_to - 1].append(
                crate_stacks[crate_row_from - 1].pop()
            )
